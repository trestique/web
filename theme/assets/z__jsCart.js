...
Shopify.theme.jsCart = {
  init: function init($section) {
    ...
    $(document).on('click', '[data-cart-page-delete]', function (e) {
      e.preventDefault();
      e.preventDefault();
      let item = e.target.closest('.cart__card')
      let bid = item.dataset?.bundleId || false;
      let mainParent = e.target.closest('.cart__form')
      $('.ajax-cart__delete').addClass('disabled')
      if(bid) {
        let qtyArray = [];
        let bundledProducts = [...mainParent.querySelectorAll(`.cart__card`)]
        ...
        Shopify.theme.jsCart.updateFromCart({updates: qtyArray},lineID);
        return;
      } else {
        var lineID = $(this).parents('[data-line-item]').data('line-item');
  ...
  updateFromCart: function updateFromCart(data, lineID) {
    ...
    $.ajax({
      ...
      success: function success(cart) {
        ...
        $('.ajax-cart__delete').removeClass('disabled')
  ...
  updateView: function updateView(cart, lineID) {
    if (cart.item_count > 0) {
      ...
      $.ajax((_$$ajax = {
        ...
        if (lineID !== null) {updatedCartList
