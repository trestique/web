...
Shopify.theme.jsAjaxCart = {
  init: function init($section) {
    ...
    $(document).on('click', '[data-ajax-cart-delete]', function (e) {
      e.preventDefault();
      let item = e.target.closest('.ajax-cart__product')
      let bid = item.dataset?.bundleId || false;
      let mainParent = e.target.closest('.ajax-cart')
      if(bid) {
        let qtyArray = [];
        let bundledProducts = [...mainParent.querySelectorAll(`.ajax-cart__product`)]
        bundledProducts.forEach((bundleItem)=>{
          if(bid == bundleItem.dataset?.bundleId){
            qtyArray.push(0)
          }else{
            qtyArray.push(bundleItem.dataset.itemQuantity)
          }
        })
        console.log("qtyArray",qtyArray);
        
        Shopify.theme.jsAjaxCart.updateFromCart({updates: qtyArray});
        return;
      } else {
  ...
  updateFromCart: function updateFromCart(data, callback) {
    $.ajax({
      type: 'POST',
      url: '/cart/update.js',
      data: data,
  ...
  updateView: function updateView() {
    ...
    'ajax' // view name (suffix)
      ...
      if (options.item_count > 0) {
        ...
        $('.ajax-cart__details-wrapper').removeClass('is-hidden');
        ...
        $('.ajax-cart__details-wrapper').addClass('is-hidden');
x
